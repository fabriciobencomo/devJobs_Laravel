        ...
        <div class="w-full max-w-sm">
            <div class="flex flex-col breaks-words bg-white border border-2 shadow-md mt-10">
                <div class="bg-gray-300 text-gray-700 uppercase text-center py-3 px-3">
                    ...
                    <form class="px-3 py-5" method="POST" action="{{ route('password.email') }}" novalidate>
                        @csrf
                        @if(session('status'))
                        <span class="bg-red-100 border-l-4 border-red-500 text-red-700 p-4 w-full mt-5 text-sm block my-5" role="alert">
